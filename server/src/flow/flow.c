...
void notify_leader(int leader, entity** players, int new_player){
    ...
    int players_num=0;
    for(int i=0; i<5; i++){
        if(players[i] && players[i]->is_player && players[i]->jugador->client_socket==new_player){
            sprintf(msg, "Nuevo jugador: %s - %s\n", players[i]->jugador->nombre, players[i]->type);
            // se cuentan los con tipo==los ready
            if(players[i]->type){
                players_num+=1;
            }
        }
    }
    // Notificar nuevo jugador
...
bool game(entity** players, int leader,int connections){
    ...
    if(!type){
        if(connections == 4){
            ...
            printf("Maximo de conexiones alcanzado\n");
...
void avisos_lider(entity** entities, char* aviso){
    int leader;
    bool theres_leader = false;
    for(int i=0; i<5; i++){
        if(entities[i] && entities[i]->is_player && entities[i]->jugador->party_leader){
            leader = entities[i]->jugador->client_socket;
            theres_leader = true;
        }
    }
    if(theres_leader){
        server_send_message(leader, 79, aviso);
    }
}
